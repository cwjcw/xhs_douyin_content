...
class Dividend:
    ...
    def video_dividend(self):
        ...
        参数:
        self.video_df: 包含['笔记标题', '观看量', '点赞', '收藏量, '评论', '分享']的数据
        ...
        # 按笔记标题汇总总表现分
        video_scores = video_df.groupby('笔记标题', as_index=False)['总表现分'].sum()
        ...
        return video_scores[['笔记标题', '总分成', '日期']]
    ...
    def everyone_money(self):
        ...
        # 统一关键字段名称
        video_people = video_people.rename(columns={"正片标题": "笔记标题"})

        # **1️⃣ 数据合并**
        merged = video_people.merge(video_money, on="笔记标题", how="left")
        ...
        if not missing_rules.empty:
            ...
            print(missing_rules[["笔记标题", "人员类别", "是否完整内容"]])

        merged = merged.dropna(subset=["分成比例"])  # 确保 `分成比例` 不能为空

        # **6️⃣ 计算 `人数`**
        merged["人数"] = merged.groupby(["笔记标题", "人员类别"])["人员"].transform("count")

        # **7️⃣ 计算 `分成金额`**
        merged["分成金额"] = (merged["总分成"] * merged["分成比例"] / merged["人数"]).round(2)

        # **8️⃣ 过滤有效数据（不丢失 `分成金额 = 0` 的数据）**
        result = merged.loc[(merged["人员"].notnull()), ["笔记标题", "人员", "分成金额"]]
