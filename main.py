...
from data_processing import dividend_processor as dy_bjlp_money
...
# 调用并发库asyncio执行批量上传
people_money = dy_bjlp_money.everyone_money()
...
# 调用并发库asyncio执行批量上传
video_money = dy_bjlp_money.video_dividend()
asyncio.run(jdy.batch_create(app_id=appId1, entry_id=entryId1, source_data=video_money))
